...
@endif
    <div class="card">
      <div class="card-header">
        <h4>Ajouter les elements qui composent votre repas </h4>
      </div>
      <div class="card-body">
    ...
    <br>
    <input type="text" value="{{ $composants }}" id="composants">
    <form action="{{ route('post_form_view') }}" method="POST" id="add_repas">
        @csrf
        <div id="selectContainer" ></div>
        <div id="inputContainer"></div>
        <br>
        <input type="submit" value="Enregistrer"  class="btn btn-warning delete-confirm" id="enregistrer">
    </form>
        <script>
const addSelectButton = document.getElementById('addSelectButton');
const selectContainer = document.getElementById('selectContainer');

var valeurEnJavaScript = document.getElementById('composants');
    valeurEnJavaScript = JSON.parse(valeurEnJavaScript.value);
    var tableauCp = [];


    for(var i = 0; i < valeurEnJavaScript.length;i++){
            tableauCp.push(valeurEnJavaScript[i].composant_repas);
    }
...
document.getElementById('add_repas').addEventListener('submit', function (event) {
        event.preventDefault();
        swal({
            title: 'Avez-vous deja ajouter les composants de votre repas?',
            text: 'cliquez sur oui pour enregsitrer le repas.',
            icon: 'warning',
            buttons: ["Annuler", "Oui!"],
        }).then(function(value) {
            if (value) {
